...
async function handlePickFile(e) {
  ...
  document.getElementById('pickedFile').textContent = pickedUploadPath.split('/').pop();
}
...
async function createPost() {
  ...
  const res = await fetch(API.posts+'?action=create', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) });
  const data = await res.json();
  if (!data.success) return alert(data.message || 'Failed to post');
  pickedUploadPath = '';
  document.getElementById('pickedFile').textContent = '';
