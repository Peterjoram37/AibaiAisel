...
// Calls storage
function calls_path() { return data_dir() . '/calls.json'; }
...
function load_calls() { return read_json(calls_path()); }
function save_calls($rows) { return write_json(calls_path(), $rows); }

function find_user_by_email_or_username($list, $login) {
  foreach ($list as $u) {
    if (strcasecmp((string)($u['email'] ?? ''), (string)$login) === 0 ||
        strcasecmp((string)($u['username'] ?? ''), (string)$login) === 0) {
      return $u;
    }
  }
  return null;
...
function append_warning($userId, $text, $type='warning') { add_notification($userId, $type, '', $text); return true; }

function get_wallet(&$wallets, $userId) {
  foreach ($wallets as &$w) if ((string)($w['userId']??'') === (string)$userId) return $w;
  $w = ['userId'=>(string)$userId,'coins'=>0,'updatedAt'=>date('Y-m-d H:i:s')];
  $wallets[] = $w;
  return $wallets[array_key_last($wallets)];
}

// Additional functions for profile functionality
...
function get_user_posts($userId) {
    ...
    foreach ($posts as $post) {
        if ($post['userId'] === $userId) {
            // Add user info
            $user = get_user_by_id($users, $post['userId']);
            $post['user'] = $user;
